...
//     ]
// };
// import { select, addEvent, debounce, addBrackets } from "../utils.js"
export const utils = {
    ...
    select: (...selectors) => selectors.map(selector => document.querySelector(selector)),

    // selectAll() usage: const [sel1, sel2] = selectAll(".class1", ".class2")
    // wrapper around querySelectorAll
    selectAll: (...selectors) => selectors.map(selector => document.querySelectorAll(selector)),

    // addEvent() usage: addEvent(element, handler, eventType)
    // wrapper around addEventListener with optional event type at the end
    addEvent: (element, handler, eventType = "click") => element.addEventListener(eventType, handler),
...
const { addEvent, debounce, addBrackets } = { ...utils };
